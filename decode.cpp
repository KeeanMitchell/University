...
int main()
{
    ...
    {
        pair<char, int> temp;
        temp.first = characters[i];
        temp.second = freq[i];
        freqTable.push_back(temp);
    }
    list<TreeNode *> *q = prepareQ(freqTable.size(), freqTable);
    ...
    // map<char, string> theDict;
    // for (int i = 0; i < freq.size(); i++)
    // {
    //     pair<char, string> entry;
    //     entry.first = characters[i];
    //     entry.second = to_string(freq[i]);
    //     theDict.insert(entry);
    // }
    // for (map<char, string>::const_iterator it = theDict.begin(); it != theDict.end(); it++)
    // {
    //     cout << it->first << ' ' << it->second << endl;
    // }
